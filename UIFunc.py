...
class UIFunc(QMainWindow, Ui_UIView, QtStyleTools):
    ...
    def __init__(self, app):
        ...
        self.bt_create_task.clicked.connect(self.OnCreateTaskButton)
    ...
    def onchangelang(self):
        ...
        # 初始化任务调度器
        self.task_scheduler = TaskScheduler(to_abs_path('tasks'))
    ...
    def closeEvent(self, event):
        ...
        if self.runthread:
            # 请求线程停止
            self.runthread.update_state(State.IDLE)
            # 确保线程不会因暂停而阻塞
            self.runthread.resume()
            logger.info('Run thread stopped')
    ...
    def OnCreateTaskButton(self):
        """
        创建任务按钮事件
        """
        # 获取默认宏文件目录（exe所在目录的scripts文件夹）
        from KeymouseGo import to_abs_path
        script_dir = to_abs_path('scripts')
        macro_files = []
        ...
        from UITaskDialogFunc import UITaskDialogFunc
        
        # 添加一个按钮让用户选择宏文件
        if QMessageBox.question(self, "选择宏文件", "是否从其他文件夹选择宏文件?", 
                                QMessageBox.Yes | QMessageBox.No) == QMessageBox.Yes:
            file_paths, _ = QFileDialog.getOpenFileNames(self, "选择宏文件", ".", "JSON5 Files (*.json5)")
            if file_paths:
                macro_files = [os.path.basename(file) for file in file_paths]
                # 保存用户选择的宏文件路径
                self.selected_macro_paths = file_paths
            else:
                # 如果用户取消选择，则使用默认路径的宏文件
                self.selected_macro_paths = None
        else:
            self.selected_macro_paths = None
        
        # 准备宏文件路径映射
        macro_paths = {}        
        if hasattr(self, 'selected_macro_paths') and self.selected_macro_paths:
            for file_path in self.selected_macro_paths:
                file_name = os.path.basename(file_path)
                macro_paths[file_name] = file_path
        
        # 打开任务创建对话框
        dialog = UITaskDialogFunc(self, macro_files=macro_files, macro_paths=macro_paths)
        if dialog.exec() == QDialog.Accepted:
            # 任务创建成功，可以在这里添加后续处理
            task_data = dialog.get_task_data()
            QMessageBox.information(self, "Success", f"Task '{task_data['task_name']}' created successfully!")
