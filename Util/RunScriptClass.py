...
class RunScriptMeta:
    ...
    def sleep(self, msecs: int, stop_check=None):
        """
        非阻塞式睡眠方法
        :param msecs: 睡眠时间(毫秒)
        :param stop_check: 停止检查函数，返回True表示应停止睡眠
        """
        ...
        while remaining > 0:
            ...
            mutex.lock()
            cond.wait(mutex, QDeadlineTimer(int(sleep_time)))
            mutex.unlock()
...
class RunScriptClass(QThread, RunScriptMeta):
    ...
    def sleep(self, msecs: int):
        """
        RunScriptClass的睡眠方法，会检查暂停状态
        """
        def stop_check():
            return self.eventPause or self.state == State.IDLE
        RunScriptMeta.sleep(self, msecs, stop_check)
...
class RunScriptCMDClass(QThread, RunScriptMeta):
    ...
    def sleep(self, msecs: int):
        """
        RunScriptCMDClass的睡眠方法，会检查停止标志
        """
        def stop_check():
